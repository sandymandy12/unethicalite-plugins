...
from constants import INNER_PATH, PLUGIN_GRADLE_TEMPLATE, CONFIG_TEMPLATE, PLUGIN_TEMPLATE
...
def load_params(argv):
    ...
    try:
        opts, argv = getopt.getopt(
            argv, "hn:f:d:", ["name=", "files=", "description="])
    except getopt.GetoptError:
        print('createplugin.py -n <name> -p <files> -d <description>')
        sys.exit(2)

    for opt, arg in opts:
        if opt == '-h':
            print('createplugin.py -n <name> -p <package> -d <description>')
            print("\nPackages are comma separated: eg:\n")
    ...
    return {"name": plugin_name, "files": files, "description": description}
...
# <----------------------- [ creating gradle file] ----------------------------------------->
    print("creating gradle file info...")
...
# <----------------------- [ create plugin files] ----------------------------------------->
    print("generating plugin file info...")
    ...
    for i in params["files"]:
        ...
        if str(i).lower() == "plugin":
            ...
            parsed_template = parse(
                "package dev.hoot.LOWERCASED;", plugin_name)
